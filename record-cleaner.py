...
class Route53ops(object):
    ...
    def get_records(self, hostedzones):
        self.hostedzones = hostedzones

        for hz in self.hostedzones:
            hz = hz['Id'].replace('/hostedzone/', '')

            max_records = 10
            dns_records = []
            
            print("Hosted Zone: {0}".format(hz))
...
class ENIops(object):
    ...
    def get_enis(self):
        ...
        for eni in self.enis['NetworkInterfaces']:
            yield {'ip_address': eni['PrivateIpAddress'], 'eni_id': eni['NetworkInterfaceId']}
...
if __name__ == '__main__':
    ...
    records = r.get_records(hostedzones)
    record_values = []
    for rr in records:
        for value in rr['ResourceRecords']:
            record_values.append(value['Value'])
            if value['Value'] not in enis:
                print("Found stale Route53 record: {0} - {1}".format(value['Value'], rr['Name']))
